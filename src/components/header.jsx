...
export default function Header() {
  ...
  const toRender = user ? (
    <Logged
      ...
      name={name}
      setBearer={setBearer}
      clearCookie={handleCookieDelete}
    />
  ) : (
    <Login />
  );
  return (
    <div>
      <div className="Header">
        <div className="log">{toRender}</div>
      </div>
      <NavBar />
    </div>
  );
...
function Logged(props) {
  return (
    <div className="logs">
      <a
        href={`${process.env.REACT_APP_ORIGIN_PAGE}/profile`}
        className="log-link"
      >
        Welcome, {props.name}
      </a>

      <a href={`${process.env.REACT_APP_ORIGIN_PAGE}/profile`}>
        <img
          src={props.userImg}
          width="50px"
      ...
      <button className="logout-link" onClick={props.clearCookie}>
        log Out
      </button>
    </div>
  );
