...
export default function Post() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const getPost = async () => {
        await axios
          .get(`${process.env.REACT_APP_API}/post/${id}`, {
            headers: {
      ...
      const retrieveToken = async () => {
        try {
          await axios
            .get(`${process.env.REACT_APP_API}/token`, {
              headers: {
                "Content-Type": "application/json",
                Accept: "application/json",
              },
      ...
      const getUser = async () => {
        try {
          if (bearer) {
            console.log("its passing away");
            const retrieveUser = await axios
  ...
  return (
    <div className="article-section top-article">
      ...
      {post["date"] ? (
        <p className="date">
          {" "}
          Sun fix, {post["date"].split("-")[2].slice(0, 2)}/
          {post["date"].split("-")[1]}/{post["date"].split("-")[0]}{" "}
        </p>
      ) : (
        <div
          style={{
      ...
      <div className="user-id">
        ...
        <p className="user-bio">
          Artista, escritora, más de 10 años en el entretenimiento Artista,
          escritora, más de 10 años en el entretenimiento
        </p>
      ...
      {localStorage.getItem("userComment") ? (
        <div className="comments-wrapper">
          <textarea
            ...
            value={textArea}
          />
          <button className="post-button2" onClick={handleComment}>
            Post
          </button>
        </div>
      ) : (
        <div className="comments-wrapper-login">
          <span>
            <a
              href={`${process.env.REACT_APP_ORIGIN_PAGE}/login`}
              className="log-link2"
            >
              Login
            </a>{" "}
            /{" "}
            <a
              href={`${process.env.REACT_APP_ORIGIN_PAGE}/register`}
              className="log-link2"
            >
              Sign In
            </a>
