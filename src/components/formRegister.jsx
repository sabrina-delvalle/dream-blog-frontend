...
class FormRegister extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      password: "",
      profilePic:
        "https://res.cloudinary.com/diyvxyidy/image/upload/v1671549902/users/user_bh6ggf.png",
      alertMessage: "",
  ...
  handleImage(event) {
    ...
    axios
      .post(`${process.env.REACT_APP_API}/profilepic`, formData, {
        headers: {
  ...
  handleUsername(event) {
    ...
    fetch(`${process.env.REACT_APP_API}/checkuser`, {
      ...
      .then((data) => {
        data.valid
          ? this.setState({ validUsername2: true })
          : this.setState({ validUsername2: false });
  ...
  handleEmail(event) {
    ...
    if (lastChar.some((e) => e === "@")) {
      lastCharEmail[1].split("").some((e) => e === ".") &&
      lastCharEmail[1].split(".")[1].length >= 2
    ...
    fetch(`${process.env.REACT_APP_API}/checkuser`, {
      ...
      .then((data) => {
        data.valid
          ? this.setState({ validEmail2: true })
          : this.setState({ validEmail2: false });
        //console.log('data from user server: ', data);
      });
  }
  ...
  handleSubmit(event) {
    ...
    if (this.state.username.length < 6 && this.state.password.length < 8) {
      return this.setState({
        alertMessage:
          "Please verify: username should be more than 6 chars and password more than 8 chars.",
      });
    } else if (this.state.username.length < 6 && this.state.validUsername2) {
      return this.setState({
        alertMessage: "Please verify: user already exist",
      });
    } else if (this.state.password.length < 8) {
      return this.setState({
        alertMessage: "Please verify: password should be more than 8 chars.",
      });
    } else if (!this.state.validEmail2) {
      return this.setState({
        alertMessage: "Please verify: email already exist.",
      });
    }
    ...
    const postRegister = async () => {
      try {
        const awaitRegister = await axios
    ...
    return (
      <div className="register">
        <form onSubmit={this.handleSubmit} className="centerForm">
          ...
          <label className="blocks" name="name">
            {" "}
            Name{" "}
          </label>
          <input
            ...
            name="name"
          />
          <label className="blocks" name="lastname">
            {" "}
            Lastname{" "}
          </label>
          <input
            ...
            name="lastname"
          />
          <label className="blocks" name="username">
            {" "}
            Username{" "}
          </label>
          <input
            ...
            className={
              this.state.validUsername1
                ? this.state.validUsername2
                  ? "input-3"
                  : "input-2"
                : "input-4"
            }
            name="username"
          />
          <label className="blocks" name="email">
            {" "}
            Email{" "}
          </label>
          <input
            ...
            className={
              this.state.validEmail1
                ? this.state.validEmail2
                  ? "input-3"
                  : "input-2"
                : "input-4"
            }
          ...
          />
          <label className="blocks" name="password">
            {" "}
            Password{" "}
          </label>
          <input
            ...
            className={this.state.validPassword1 ? "input-3" : "input-4"}
            name="password"
            placeholder="********"
          />
          <img
            ...
            className={
              this.state.validProfilePic ? "img-selector-off" : "img-selector"
            }
            width="130px"
          ...
          <div
            className={
              this.state.validProfilePic ? "loader-pic-on" : "loader-pic"
            }
          >
            <TailSpin
              height="80"
          ...
          <div className="img-btn-wrapper">
            <label
              className={
                this.state.validProfilePic ? "imageBtn-off" : "imageBtn"
              }
              name="profileImage"
...
export function RegisterWithLogin(props) {
  ...
  return <FormRegister navigate={navigate} />;
}
