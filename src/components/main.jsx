...
export default function Main() {
  ...
  let arts = article.map((elem) => (
    <li key={article.indexOf(elem)} className="top-article">
      <a
        href={`${process.env.REACT_APP_ORIGIN_PAGE}/post/${elem["_id"]}`}
        className="postLink"
      >
      ...
      <p className="date">
        {" "}
        Sun, {elem["date"].split("-")[2].slice(0, 2)}/
        {elem["date"].split("-")[1]}/{elem["date"].split("-")[0]}
      </p>
      <div className="post-article">
        {parse(draftToHtml(elem["article"]))[0]}
      </div>
